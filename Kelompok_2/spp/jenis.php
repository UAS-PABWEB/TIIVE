...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
      ...
      echo '<div class="row">';
		...
		echo '<tr class="info"><th>No</th><th width="200">Tahun Pelajaran</th><th>Tingkat</th><th>Jumlah Nominal</th>';
		...
		if(mysql_num_rows($sql) > 0){
			...
			while(list($tapel,$tingkat,$jumlah) = mysql_fetch_array($sql)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$tapel.'</td>';
				echo '<td>'.$tingkat.'</td><td>Rp <span class="pull-right">'.$jumlah.'</span></td><td>';
